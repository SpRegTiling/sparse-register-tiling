...
def load_mapping(id):
    ...
    patterns = set()
    max_mapped = 0
    with open(SCRIPT_DIR + "/mappings/mapping_{}.txt".format(id), "r") as f:
        f.readline()
        for line in f.readlines():
            line = line.split(':')
            P = int(line[0])
            max_mapped = max(max_mapped, P)
            for kern in json.loads(line[1]):
                mapping[P].append(kern)
                patterns.add(kern)

    M_r = int(math.log2(max_mapped))
    acc = {
        "M_r": M_r,
        "N_r": {
            "avx512": 16 // M_r,
