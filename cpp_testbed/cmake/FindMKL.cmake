...
#  MKL_ROOT_PATH - the variable to the library given by the user (not needed if INTEL_ONEAPI_ROOT_PATH is set)
#  INTEL_ONEAPI_ROOT_PATH - the variable to the intel oneapi directory (not required if you are not using the intel compiler)

# Do nothing if MKL_FOUND was set before!
IF (NOT MKL_FOUND)

  LIST(APPEND CMAKE_INCLUDE_PATH /usr/include/mkl)
  ...
  # Check the environment for paths to MKL (MKL_ROOT_PATH set in CMake takes precedence)
  IF(NOT MKL_ROOT_PATH)
    IF (DEFINED ENV{MKL_ROOT_PATH})
      SET(MKL_ROOT_PATH $ENV{MKL_ROOT_PATH})
    ELSEIF(DEFINED ENV{MKLROOT})
      SET(MKL_ROOT_PATH $ENV{MKLROOT})
    ENDIF()
  ENDIF()

  # Check the environment for paths to Intel OneApi (INTEL_ONEAPI_ROOT_PATH in CMake takes precedence)
  IF(NOT INTEL_ONEAPI_ROOT_PATH)
    IF (DEFINED ENV{INTEL_ONEAPI_ROOT_PATH} AND EXISTS $ENV{INTEL_ONEAPI_ROOT_PATH})
      SET(INTEL_ONEAPI_ROOT_PATH $ENV{INTEL_ONEAPI_ROOT_PATH})
    ELSEIF (DEFINED ENV{ONEAPIROOT} AND EXISTS $ENV{ONEAPIROOT})
      SET(INTEL_ONEAPI_ROOT_PATH $ENV{ONEAPIROOT})
    ELSE()
      SET(INTEL_ONEAPI_ROOT_PATH "/opt/intel/oneapi")
    ENDIF()
  ENDIF()

  # Check the OneApi root path has been
  IF(INTEL_ONEAPI_ROOT_PATH)
    IF(EXISTS ${INTEL_ONEAPI_ROOT_PATH})
      SET(DEFAULT_INTEL_COMPILER_DIR "${INTEL_ONEAPI_ROOT_PATH}")
      # If the OneApi root path is given try to use bundled mkl
      IF(EXISTS ${INTEL_ONEAPI_ROOT_PATH}/mkl/latest)
        SET(DEFAULT_INTEL_MKL_DIR "${INTEL_ONEAPI_ROOT_PATH}/mkl/latest")
      ENDIF()
    ENDIF()
  ENDIF()
  ...
  IF(NOT DEFAULT_INTEL_COMPILER_DIR)
    IF (WIN32)
      ...
      ELSE()
        SET(DEFAULT_INTEL_COMPILER_DIR "C:/Program Files (x86)/IntelSWTools/compilers_and_libraries/windows")
      ENDIF()
    ELSE()
      SET(DEFAULT_INTEL_COMPILER_DIR "/opt/intel/oneapi")
    ENDIF()
  ENDIF()
  IF(NOT DEFAULT_INTEL_MKL_DIR)
    SET(DEFAULT_INTEL_MKL_DIR "${DEFAULT_INTEL_COMPILER_DIR}/mkl")
  ENDIF()

  MESSAGE("-- Using MKL dir: ${DEFAULT_INTEL_MKL_DIR}")
  IF(DEFAULT_INTEL_COMPILER_DIR)
    MESSAGE("-- Using intel compiler dir: ${DEFAULT_INTEL_COMPILER_DIR}")
  ENDIF()
  ...
  ELSE (MKL_LIBRARIES AND MKL_INCLUDE_DIR)
    if (MKL_LIBRARIES AND NOT MKL_INCLUDE_DIR)
      ...
      directory containing them, e.g., by setting MKL_ROOT_PATH, INTEL_ONEAPI_ROOT_PATH, MKLROOT, or ONEAPIROOT.")
    endif()
    SET(MKL_FOUND FALSE)
