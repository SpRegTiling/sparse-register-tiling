...
//        _a_row_times_b_scalar<Scalar, Index, row_swizzle>(
...
static inline int _inner_loop_tiled_##TILE_SIZE_IN_VECTORS (                                                \
    ...
    for (int ii = i; ii < std::min(i + config.mTile, m); ii++) {                                            \
        _a_row_times_b_col_panel_##TILE_SIZE_IN_VECTORS<VecType, StorageTypes, row_swizzle, false>(         \
            m, k, n,                                                                                        \
            ii, j,                                                                                          \
            row_indices, values, row_offsets, column_indices,                                               \
            B, C                                                                                            \
...
void _kernel(
    ...
    using ScalarType = typename StorageTypes::Scalar;
    using VecType = typename Vec<ScalarType, vector_width>::Type;
...
INSTANTIATE_FOR_ALL_STORAGE_CONFIGS(spmm_csr_c_2d, Config);
