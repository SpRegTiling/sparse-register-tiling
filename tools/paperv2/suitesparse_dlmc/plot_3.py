...
fig, axs = plt.subplots(1, 4, figsize=(16,5.5), squeeze=False, width_ratios=[1.5, 2, 1.1, 2])
...
ax.set_ylim(limits)
ax.text(0.2, -0.285, "(a)", transform=ax.transAxes, size=18)
...
ss_df = pd.read_csv(RESULTS_DIR + "/cachev2/suitesparse_dlmc/ss_with_cov_and_avg.csv")
...
dl_df = pd.read_csv(RESULTS_DIR + "/cachev2/suitesparse_dlmc/dlmc_with_cov_and_avg.csv")
...
dl_df = filter(dl_df, Bcols=128)
ss_df = filter(ss_df, Bcols=128)
...
ax.text(0.21, -0.285, "(b)", transform=ax.transAxes, size=18)
...
ax.spines.top.set_visible(False)    
ax.set_ylim(limits)
ax.text(-0.2, -0.285, "(c)", transform=ax.transAxes, size=18)


cmap_reversed = matplotlib.cm.get_cmap('cividis_r')
mcl = mcl[2:]
...
ss = df["ss"]
df.loc[~ss & (df["density"] < 0.05), "density"] = 0.05 # normally we round so visually correct so minor outliers on the vertical line
s = ax.scatter(x=df[~ss]["avg_nnz_per_enumb|Sp. Reg."], y=df[~ss]["cov|Sp. Reg."], c=df[~ss]["gflops/s|Sp. Reg."], s=MARKSIZE_DL, marker=DL_MARKER, cmap=cmap_reversed, vmin=0, vmax=500, label="DLMC")
ax = df[ss].plot(kind='scatter', x="avg_nnz_per_enumb|Sp. Reg.", y='cov|Sp. Reg.', c='gflops/s|Sp. Reg.', ax=ax, s=MARKSIZE, marker=SS_MARKER, colormap=cmap_reversed, vmin=0, vmax=500, label="SuiteSparse", colorbar=False)

ax.set(xscale="linear", yscale="log")
ax.set_xlabel('Avg NNZ per Enum. Block')
ax.set_ylabel('CoV Working Set Size')
#ax.set_title(f'500 DLMC (60%-95%) & 500 SuiteSparse', pad=10)
#ax.axvline(x = 0.05, color = 'firebrick', linewidth=0.5)
ax.spines[['right', 'top']].set_visible(False)
#ax.text(0.25, -0.27, "(b)", transform=ax.transAxes, size=18)
ax.text(-0.05, -0.285, "(d)", transform=ax.transAxes, size=18)
...
savefig(f"/suitesparse_all_3.pdf")
