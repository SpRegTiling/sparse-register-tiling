...
s = ax.scatter(x=df[~ss]["avg_nnz_per_enumb|Sp. Reg."], y=df[~ss]["cov|Sp. Reg."], c=df[~ss]["gflops/s|PSC"], s=MARKSIZE_DL, marker=DL_MARKER, cmap=cmap_reversed, vmin=0, vmax=500, label="DLMC")
print(ax)
ax = df[ss].plot(kind='scatter', x="avg_nnz_per_enumb|Sp. Reg.", y='cov|Sp. Reg.', c='gflops/s|PSC', ax=ax, s=MARKSIZE, marker=SS_MARKER, colormap=cmap_reversed, vmin=0, vmax=500, label="SuiteSparse", colorbar=False)

ax.set(xscale="linear", yscale="log")
ax.set_xlabel('Avg NNZ per Enumerated Block')
...
cb = plt.colorbar(s)
cb.ax.set_ylabel('Required GFLOP/s (LCM I/E)')

plt.gca().xaxis.set_major_formatter(StrMethodFormatter('{x:g}'))
plt.gca().xaxis.set_minor_formatter(NullFormatter())

legend_elements = [Line2D([0], [0], marker=SS_MARKER, color='black', label='SuiteSparse',
                           linestyle='None',
                          markerfacecolor='black', markersize=5),
                   Line2D([0], [0], marker=DL_MARKER, color='black', label='DLMC',
                           linestyle='None',
                          markerfacecolor='black', markersize=5)]
...
savefig(f"/suitesparse_all_2.pdf")
