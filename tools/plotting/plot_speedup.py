...
files = [
    ...
    "transformer_magnitude_70_hybrid_ie_packed_results.csv",
    "transformer_magnitude_80_hybrid_ie_packed_results.csv",
]
...
for file in files:
    ...
    def speedup_line_vs_bcols(n):
        ...
        return alt.Chart(
            df_to_chart,
            title=[f'{model}, {pruning}, {sparsity}', f'(numThreads: {n}, matrices: {df["matrixPath"].nunique()})']
        ).mark_line(point=True).encode(
...
def create_chart_grid(name, charts, row_width):
    charts_merged = None
    charts_row = None
    ...
    altair_saver.save(charts_merged, PLOT_DIR + f'{name}.png',
                      fmt="png", scale_fator=4)
