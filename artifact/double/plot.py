...
def plot_main():
    ...
    for i in range(len(numThreadsList)):
        for j in range(len(bColsList)):
            ...
            #    `df["time median|MKL_Sparse"] / min(df["time median|ASpT"], df["time median|ASpT_increased_parallelism"])`
            # in the final paper
            for method in methods:
                df[f'Speed-up {method} vs. {baseline}'] = df[f"time median|{baseline}"] / df[f"time median|{method}"]
